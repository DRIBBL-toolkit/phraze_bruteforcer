...
const board_json_1 = __importDefault(require("./board.json"));
...
(async () => {
    ...
    console.log(UUID);
    for (let i = config_json_1.nextIndex; i < board_json_1.default.length; i++) {
        const word = board_json_1.default[i].toLocaleLowerCase();
        const guessIsCorrect = await (0, api_1.checkSideQuest)(authToken, 'blue', UUID, word);
        console.log(word, guessIsCorrect);
        if (guessIsCorrect) {
            console.log(`${word} is correct!`);
            await fs_1.promises.writeFile('./answer.json', JSON.stringify({ nextIndex: i + 1, answer: word }, null, 2), 'utf8');
            break;
        }
        if (i % 50 === 0) {
